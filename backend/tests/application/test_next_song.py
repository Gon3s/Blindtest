...
def _room_mock(room_id: UUID, host_id: UUID) -> MagicMock:
...
def test_non_last_reveal_round_not_finished(
    host_id: UUID, room_id: UUID, round_id: UUID
) -> None:
    ...
    room = _room_mock(room_id, host_id)
    sess = _build_session(current, all_songs, round_, room, [])
    result = RoomService(sess).reveal_song(current.id, host_id)
...
def test_intermediate_reveals_do_not_finish_round(
    host_id: UUID, room_id: UUID, round_id: UUID, song_index: int
) -> None:
...
def test_last_song_reveal_sets_round_status_finished(
    ...
    room = _room_mock(room_id, host_id)
    sess = _build_session(current, all_songs, round_, room, [])
    RoomService(sess).reveal_song(current.id, host_id)
